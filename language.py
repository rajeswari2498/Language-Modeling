...
def generateTextFromBigrams(count, startWords, startWordProbs, bigramProbs):
    string=""
    last_word=random.choices(startWords,weights=startWordProbs)[0]
    string=last_word
    for i in range(count-1):
        if(last_word!="."):
            word=bigramProbs[last_word]["words"]
            probability=bigramProbs[last_word]["probs"]
            last_word=random.choices(word,weights=probability)[0]
            string+=" "+last_word
        else:
            last_word=random.choices(startWords,weights=startWordProbs)[0]
            string+=" "+last_word
...
def graphTop50Words(corpus):
    ...
    unigram_probs=buildUnigramProbs(unique_words,unigram_counts,totalCount=count)
    top=getTopWords(50,unique_words,unigram_probs,ignore)
    # print(top)
...
def graphTopStartWords(corpus):
    ...
    start_word_probs=buildUnigramProbs(start_words,start_word_counts,len(corpus))
    count=getTopWords(10,start_words,start_word_probs,ignore)
    barPlot(count,"Top start Words")
    return
...
def graphTopNextWords(corpus, word):
    ...
    bigram_probs=buildBigramProbs(unigram_counts,bigram_counts)
    count=getTopWords(10,bigram_probs[word]["words"],bigram_probs[word]["probs"],ignore)
